...
@interface pushView()<CAAnimationDelegate>
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.myblurEffect.frame = CGRectMake(0, 0, ScreenWidth, ScreenHeight-64);
...
-(void)pushButton
{
    ...
    if(!self.isAnimation)
    {
        ...
        for (NSInteger i = 0; i < self.btnArray.count; i++) {
            ...
            //按钮要到的位置的frame
            CGRect toValue =CGRectMake(margin*(i+1)+width*i+width/2, ScreenHeight*0.7+width/2-64, width, width);
